...
export function TitleDialog({ isOpen, title, onClose, onSubmit }: TitleDialogProps) {
  const {
    control,
    handleSubmit,
    reset,
  ...
  // Resetear formulario cuando se abre/cierra el dialog o cambia el título
  useEffect(() => {
    if (isOpen) {
      if (title) {
        // Modo edición: cargar datos del título existente
        reset({
          name: title.name,
