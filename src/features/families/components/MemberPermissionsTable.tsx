...
type ColumnDef = {
  key: MemberViewKey;
  labelKey: 'persons' | 'events' | 'photos' | 'stories' | 'documents';
  dtoKey: keyof NonNullable<Parameters<typeof setMemberPermissions>[0]['perms']>;
};

const COLUMNS: ColumnDef[] = [
  { key: 'can_view_persons',   labelKey: 'persons',    dtoKey: 'canViewPersons' },
  { key: 'can_view_events',    labelKey: 'events',     dtoKey: 'canViewEvents' },
  { key: 'can_view_photos',    labelKey: 'photos',     dtoKey: 'canViewPhotos' },
  { key: 'can_view_stories',   labelKey: 'stories',    dtoKey: 'canViewStories' },
  { key: 'can_view_documents', labelKey: 'documents',  dtoKey: 'canViewDocuments' },
];
...
export function MemberPermissionsTable({ familyId, members, permissions }: Props) {
  ...
  const t = useTranslations('familias.permissions.table');
  const [pending, startTransition] = useTransition();
  const [busyKey, setBusyKey] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  // Show only members who are NOT owner. Owner always sees everything.
  ...
  function handleToggle(profileId: string, col: ColumnDef, next: boolean) {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="bg-zinc-50 text-zinc-600 dark:bg-zinc-900/40 dark:text-zinc-400">
            <tr>
              <th className="px-3 py-2 text-left font-medium">{t('headers.member')}</th>
              {COLUMNS.map((c) => (
                <th key={c.key} className="px-3 py-2 text-center font-medium">
                  {t(`columns.${c.labelKey}`)}
                </th>
              ))}
              <th className="px-3 py-2 text-right font-medium">{t('headers.reset')}</th>
            </tr>
          </thead>
              ...
              return (
                <tr key={m.profile_id}>
                  ...
                  {COLUMNS.map((col) => {
                    ...
                    const label = t(`columns.${col.labelKey}`);
                    return (
                      <td key={col.key} className="px-3 py-2 text-center">
                        <input
                          ...
                          title={value ? t('canSee', { label: label.toLowerCase() }) : t('cannotSee', { label: label.toLowerCase() })}
                        />
                        {pending && busyKey === key && (
                  ...
                  <td className="px-3 py-2 text-right">
                    {hasOverride ? (
                      ...
                      <span className="text-[10px] text-zinc-400">{t('noOverride')}</span>
                    )}
                  </td>
                </tr>
              );
