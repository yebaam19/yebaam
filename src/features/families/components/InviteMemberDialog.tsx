...
export function InviteMemberDialog({ familyId }: { familyId: string }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      setOkMessage(`Invitación enviada a @${username.trim().replace(/^@/, '')}.`);
      setUsername('');
      router.refresh();
    });
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
            Invitar a un familiar por @username
          </label>
          <input
            type="text"
            value={username}
          ...
          <p className="mt-1 text-xs text-zinc-500">
            Solo usuarios existentes en Yebaam. Recibirán una invitación para unirse a esta familia.
          </p>
        </div>
        {error && (
