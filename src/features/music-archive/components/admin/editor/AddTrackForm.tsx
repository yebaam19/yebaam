...
export function AddTrackForm({ nextPosition, albumFormat, onAdd }: Props) {
  ...
  async function submit() {
    if (!file || !title.trim()) {
      setError(t('admin.addTrack.errFileTitle'));
      return;
    }
    setBusy(true);
    setError(null);
    try {
      ...
      setError(err instanceof Error ? err.message : t('admin.addTrack.errGeneric'));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="mt-3 rounded-md border border-dashed border-zinc-300 bg-zinc-50/60 p-3 dark:border-zinc-700 dark:bg-zinc-900/40">
      ...
      <div className="grid grid-cols-12 items-center gap-2 text-sm">
        ...
        >
          {busy ? t('admin.addTrack.submitting') : t('admin.addTrack.submit')}
        </button>
      </div>
      {error && (
