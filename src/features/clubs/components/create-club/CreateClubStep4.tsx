...
export const CreateClubStep4: FC<CreateClubStep4Props> = ({
  ...
  const t = useTranslations('clubes.create.step4');
    ...
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          {t('tagsLabel')} <span className="text-gray-400 text-xs">{t('tagsHint')}</span>
        </label>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          {t('rulesLabel')} <span className="text-gray-400 text-xs">{t('rulesHint')}</span>
        </label>
        <div className="flex gap-2 mb-2">
          <input
            type="text"
      ...
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        ...
        <dl className="space-y-1 text-sm">
          <div className="flex justify-between">
            <dt className="text-blue-700 dark:text-blue-400">{t('summaryName')}</dt>
            <dd className="text-blue-900 dark:text-blue-200 font-medium">{data.name}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-blue-700 dark:text-blue-400">{t('summaryCategory')}</dt>
            <dd className="text-blue-900 dark:text-blue-200">{data.category}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-blue-700 dark:text-blue-400">{t('summaryPrivacy')}</dt>
            <dd className="text-blue-900 dark:text-blue-200">{data.privacy || t('privacyDefault')}</dd>
          </div>
          {tags.length > 0 && (
            <div className="flex justify-between">
              <dt className="text-blue-700 dark:text-blue-400">{t('summaryTags')}</dt>
              <dd className="text-blue-900 dark:text-blue-200">{tags.length}</dd>
            </div>
          )}
          {rules.length > 0 && (
            <div className="flex justify-between">
              <dt className="text-blue-700 dark:text-blue-400">{t('summaryRules')}</dt>
