...
import { listLatestAlbums } from '@/features/music-archive/server/music.server';
...
export default async function MusicArchiveLandingPage() {
  const [albums, client] = await Promise.all([listLatestAlbums(24), getServerClient()]);
    ...
    <div className="mx-auto w-full max-w-6xl space-y-10 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      ...
      <section className="space-y-3">
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-xs text-zinc-500">{albums.length} álbumes</span>
        </div>
        {albums.length === 0 ? (
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {DECADES.map((d) => (
            <Link
              key={d.start}
              href={`/musica?decade=${d.start}` as Route}
              className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-700 hover:border-amber-400 hover:bg-amber-50 hover:text-amber-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:border-amber-700 dark:hover:bg-amber-900/20"
            >
              {d.label}
            </Link>
          ))}
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {COUNTRIES.map((c) => (
            <Link
              key={c.code}
              href={`/musica?country=${c.code}` as Route}
              className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-700 hover:border-amber-400 hover:bg-amber-50 hover:text-amber-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:border-amber-700 dark:hover:bg-amber-900/20"
            >
              {c.label}
            </Link>
