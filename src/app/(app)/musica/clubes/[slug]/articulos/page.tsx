...
export default async function ClubArticlesPage({
  ...
  if (!club) notFound();
  const [articles, viewerRole, joinStatus, t] = await Promise.all([
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{t('clubs.articlesHeading')}</h2>
        {canWrite && (
          <Link
          ...
          >
            <PlusIcon className="h-4 w-4" /> {t('clubs.newArticleCta')}
          </Link>
        )}
      </div>
